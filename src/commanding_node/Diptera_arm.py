...
class Arming_Modechng():
	...
	self.Liftoff_thrust_old = None
        ...
        self.moving_thrust_calculated = None
...
################################# LIFT OFF ########################################
	...
	for i in range(recursions):
            ...
            elif self.beh_type == "TAKE OFF" and thrust < 0.58:
...
################################# HOVER ########################################
	...
	for i in range(recursions):
            ...
            if self.stop_the_drone == True:
                ...
                self.moving_thrust_calculated = thrust
                self.construct_target_attitude(0,0,0,thrust, 0, 0, 0)
                #self.construct_target_attitude(0,0,0,thrust, self.angle_roll_left_right_trim, self.angle_pitch_forward_back_trim, 0)
...
#                      /// MOVE RAW ///
	...
	for i in range(recursions):
            ...
            if self.stop_the_drone == True:
                ...
                thrust = self.calculate_thrust()
                #thrust = self.moving_thrust_calculated
...
if __name__ == '__main__':
    ...
    try:
        ...
        elif arm.battery_voltage < 13:
            rospy.logerr("Low battery level, disarming")
        else:
            rospy.logerr("No lidar down detected, disarming")
